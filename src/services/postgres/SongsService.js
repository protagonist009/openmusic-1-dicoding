...
class SongsService {
    ...
    async getSongs() {
        // const result = await this._pool.query('SELECT id = $1, title = $2, performer = $3 FROM songs');
        const query = {
            text: 'SELECT id = $1, title = $2, performer = $3 FROM songs',
        ...
        if (!result.rows.length) {
            throw new NotFoundError('Lagu tidak ditemukan. Silahkan tambahkan lagu');
